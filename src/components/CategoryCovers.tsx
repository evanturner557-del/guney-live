...
export default function CategoryCovers({ stats, t }: { stats: Record<string, CatStat>; t: (k: string) => string }) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
      {CATEGORIES.map((c) => {
        const s = stats[c.key] ?? { count: 0, cover: null };
        const name = t(`gallery.cat.${c.key}.name`);
        return (
          <Link key={c.key} href={`/gallery/${c.key}`}
            className="relative aspect-[4/3] rounded-2xl overflow-hidden border border-sand group block">
            {s.cover ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={s.cover} alt={name} loading="lazy"
            ...
            <div className="absolute inset-x-0 bottom-0 p-3 text-white">
              <div className="flex items-end justify-between">
                <div>
                  <p className="text-[11px] uppercase tracking-wide text-white/70">{c.emoji} {t("gallery.cover.category")}</p>
              ...
              <p className="text-[11px] text-white/80 mt-1">
                {s.count > 0 ? `${s.count} ${s.count > 1 ? t("gallery.cover.photos") : t("gallery.cover.photo")}` : t("gallery.cover.addfirst")}
              </p>
            </div>
          </Link>
        );
